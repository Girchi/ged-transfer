        ...
        <div className="mt-6 flex flex-col gap-4">
            ...
            <div className="flex p-1 gap-0.5 w-full h-10 bg-[#292d330a] rounded-lg">
                <div onClick={() => setDealType('გაყიდვა')} className={`flex justify-center items-center p-1 gap-2.5 
                    w-[33%] h-8 rounded-[4px] ${ dealType==='გაყიდვა' ? 'text-secondaryGreen bg-white shadow-small' : 'text-lightGray' }`}>
                ...
                <div onClick={() => setDealType('ნივთის ყიდვა')} className={`flex justify-center items-center p-1 gap-2.5 
                    w-[34%] h-8 rounded-[4px] ${ dealType==='ნივთის ყიდვა' ? 'text-secondaryGreen bg-white shadow-small' : 'text-lightGray' }`}>
                ...
                <div onClick={() => setDealType('გაჩუქება')} className={`flex justify-center items-center p-1 gap-2.5 
                    w-[33%] h-8 rounded-[4px] ${ dealType==='გაჩუქება' ? 'text-secondaryGreen bg-white shadow-small' : 'text-lightGray' }`}>
            ...
            { dealType==='გაყიდვა' && (
                <div className="relative">
                    ...
                    <NumericFormat
                        className="flex justify-between items-center p-2.5 pr-10 gap-2 bg-white border-solid border-[1px] border-bgGray rounded-md 
            ...
            { dealType === 'ნივთის ყიდვა' && (
                <div className="relative">
                    ...
                    <input
                        className="flex justify-between items-center p-2.5 pr-10 gap-2 bg-white border-solid border-[1px] border-bgGray
